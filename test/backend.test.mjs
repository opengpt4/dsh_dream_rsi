...
test('executes a high-level action and exposes the updated state', () => {
  const backend = new MockEmbodiedBackend();

  const result = backend.execute('session-a', 'move_relative', { dx: 1, dy: 2, dz: 0 });
...
test('keeps action state isolated between sessions', () => {
  const backend = new MockEmbodiedBackend();

  backend.execute('session-a', 'pick', {});
