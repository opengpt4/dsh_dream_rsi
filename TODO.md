...
- [x] Extend the `dream status` facade to the rest of the FUNCTIONAL_SPEC.md §4.4 report: current policy and latest evaluation are read from the persisted registry through a view that creates nothing (`ReadOnlyFilePolicyRegistryStore`, `storage.registryDir`), an absent registry reads as empty, and a corrupt one is reported as `unreadable` with the reason. Resource *limits* are projected from the constants the isolated child is launched with; resource *consumption* is not, because nothing accounts for it per child.
...
- [x] Select first simulator and task family. RoboSuite 1.5.2 on MuJoCo 3.2.3 (`Lift` and `Stack`), chosen against the four criteria and checked on this machine — headless run, camera payloads, and the observation mapping. Determinism of the initial state is **not** established: 1.5.2 exposes no seed API (`env.seed` is `None`), so the earlier cross-process determinism claim was withdrawn — the item below and `BASELINE.md` carry that. Evidence in `BASELINE.md`; the MuJoCo pin is empirical because RoboSuite 1.5.2 fails against 3.13 while its own metadata asks for `>=3.3.0`.
- [ ] Freeze observation schema, coordinate system, action capability profile, and scoring function. `[!]`
- [ ] Implement one real `EnvironmentAdapter` over the RoboSuite bridge. The simulator is chosen and verified. The process bridge exists (`sim/robosuite_bridge.py`, eight tests in `test/robosuite-bridge.test.mjs` that skip without `DREAM_RSI_SIM_PYTHON`) and the mapping from its state to the plugin's model exists (`src/embodied/robosuite-mapping.ts`, validated field by field because the state crosses a process boundary). What remains is the adapter itself — spawn the bridge, implement the seven `EnvironmentAdapter` methods over that mapping, and the fixtures. An attempt landed and was withdrawn uncommitted; it established three things the next attempt starts from: the child's **stderr must be drained**, because robosuite logs to it and a full unread pipe deadlocks the child mid-request in a way that looks like a slow simulator; adapter-level outcomes (**cancelled**, and a failure with no reply) must **not** be routed through the bridge's status mapper, which only knows `completed`/`timeout`/`failed`; and an adapter that has been released must **reject** further requests rather than silently spawning a fresh simulator, or a caller believes an environment is gone while a new one is running. The test file also needs its budget raised or one shared child, because each spawning test pays robosuite's import again. The mapping is exported and tested but has **no production consumer until the adapter lands**, which is why `test/architecture.test.mjs` only passes because of that export rather than because something uses it. The boundary it requires is in place and now enforced: the SDK is reached only through `embodied/mock-adapter.ts`, the protocol depends on nothing but the core data model, and all three embodied tools take `EnvironmentAdapter` rather than the SDK. `test/architecture.test.mjs` fails if that changes.
- [ ] Add deterministic seed and large-payload artifact capture; both need a simulator that produces randomness or payloads. `reset` and the failure fixtures exist, and disposal now releases every session rather than only closing storage, so an environment is not left holding a pose after unload. RoboSuite 1.5.2 has no seed API (`env.seed` is `None`), so the seed half is withdrawn as recorded in BASELINE.md: what is needed is either a supported seeding path or a pinned initial state, and `test/sim/` will have to show it.
...
- [x] Add ablations for boundary penalty, task-level split, cost term, and parallel-efficiency term (`src/evolution/ablation.ts`). Each evaluator term is removed one at a time, and `scoreDelta` is the variant's score minus the baseline's, so removing a penalty raises the score and removing the parallel bonus lowers it. The task-level split is compared against a node-level one that leaks tasks across partitions. This decomposes a recorded result; it is architecture validation, not a performance claim.
