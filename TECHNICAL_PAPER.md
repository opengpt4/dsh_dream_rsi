...
The current suite contains 468 passing tests. These tests validate implementation invariants such as configuration safety defaults, session isolation, action behavior, replay determinism, storage idempotency, evaluator formulas, split determinism, gate rejection, snapshot immutability, process evaluation, lock recovery, and lifecycle disposal.
...
| Immutable signed policy/evaluation/snapshot artifacts | Partial | Ed25519 signatures over policy artifact and evaluation report ids with an SPKI PEM key ring for rotation (`src/registry/signing.ts`); activation refuses an unsigned or tampered artifact or evidence report and names the reason. Snapshots are unsigned |
