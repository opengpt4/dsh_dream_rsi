...
Partial setup failure must clean up registrations that were already created. Tested for the four tool registrations and the single effect this plugin creates, and the mount is asserted to call none of `provide`, `command`, `worker`, or `on`, so no service, command, worker, or listener lifecycle exists to test.
...
This design prevents a candidate from being evaluated against one input and deployed against a silently replaced input. Policy artifacts and evaluation reports carry detached Ed25519 signatures over their ids, verified against an SPKI PEM key ring before activation, which is what makes that check enforceable rather than advisory.
...
The current suite contains 523 passing tests, 7 of which skip unless `DREAM_RSI_SIM_PYTHON` names an interpreter with robosuite installed, so the simulator cases are covered without making the simulator a repository dependency. These tests validate implementation invariants such as configuration safety defaults, session isolation, action behavior, replay determinism, storage idempotency, evaluator formulas, split determinism, gate rejection, snapshot immutability, process evaluation, lock recovery, and lifecycle disposal.
