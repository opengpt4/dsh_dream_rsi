...
The current suite contains 460 passing tests. These tests validate implementation invariants such as configuration safety defaults, session isolation, action behavior, replay determinism, storage idempotency, evaluator formulas, split determinism, gate rejection, snapshot immutability, process evaluation, lock recovery, and lifecycle disposal.
...
| Immutable signed policy/evaluation/snapshot artifacts | Partial | Ed25519 signatures over policy artifact ids with an SPKI PEM key ring for rotation (`src/registry/signing.ts`); activation refuses an unsigned or tampered artifact and names the reason. Evaluation reports and snapshots are unsigned |
| Full OS/container sandbox | Not implemented | required before untrusted candidate execution |
...
| `dream status` management report | Partial | enabled capabilities, action-admitting controls, promotion thresholds, and readiness (`src/status.ts`); current policy, latest evaluation, and resource usage from FUNCTIONAL_SPEC.md §4.4 are not projected |
| Real simulator adapter | Not implemented | Phase 6 |
