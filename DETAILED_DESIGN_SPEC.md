...
Implemented as `verifyReadiness` (`src/readiness.ts`), surfaced through `dream_status` when the tool is constructed with a runtime; `noUnusedParameters` is on, so a runtime argument the tool ignores is a compile error rather than a silently dropped input. Each check reports `ready`, `not-ready`, or `not-probed` rather than a boolean, because opening the store creates the database file and the status tool is documented as side-effect free: storage is left `not-probed` there and probed only when a caller asks. A `not-probed` check does not make the report claim readiness.

The same report carries the configuration that decides whether an action is admitted — `embodied.requireConfirmation`, `embodied.allowActions`, `embodied.actionLeaseMs`, `embodied.maxActionsPerMinute` — and the promotion thresholds `evolution.minimumPassRatio` and `evolution.minimumImprovement`. The enabled flags alone say a capability is on, not what it permits. `allowedActions` is copied into the report, so a caller that sorts or empties it does not edit the running configuration. The report also carries the persisted registry through a view that creates nothing: `storage.registryDir` is read on every call rather than cached, because a cached view would report the state at its first read and miss every later registration. It reports counts, the current policy with its version and whether a signature is present, and the most recent evaluation report; an absent directory is an empty registry, and a corrupt one is reported as `unreadable` with the reason rather than thrown, since the status call is how an operator finds out. `verifyReadiness` receives that view, so the current-policy check runs in the tool instead of only in tests; that check reports the version and a short id prefix, and the version is printed as it is stored because it already carries its `v` prefix. The view refuses to save, so a caller that keeps it cannot write through it, and the flattened tool output keeps every key with `null` where nothing was read, which is what lets one output schema cover both states.

Resource *limits* are projected too — the heap cap, the wall-clock deadline, and the output bound — read from the constants and configuration the isolated child is launched with, so the report cannot drift from the enforcement. Resource *consumption* is not reported: nothing accounts for it per child, and a process-wide number would read as if something did.
...
export interface DreamRsiConfig {
  enabled: boolean;
  storage: { sqlitePath: string; artifactDir: string; registryDir: string };
...
Because storage is content-addressed, `artifactId` **is** the SHA-256 of the bytes: a separate `contentSha256` field would be the same value twice, and a stored `uri` is derivable from the id. `verify(artifactId)` re-hashes the blob on disk rather than trusting the index, and distinguishes `unknown`, `deleted`, `missing_blob`, and `checksum_mismatch`. It compares that digest with the id it was asked about *and* with the id the record claims: the first is the file's name, so on its own it left metadata edited to name a different artifact accepted, and a reference built from that record would have pointed at something other than the bytes it describes. `assertArtifactsVerified` fails closed before evaluation; deployment must call it rather than re-implement the check.

Retention and deletion are recorded in metadata, never by rewriting or truncating a blob. One blob carries one record, so the same bytes cannot be filed under two kinds: the second kind, and the retention the policy derives from it, would silently become the first writer's. Storing a policy after the same bytes had been stored as an observation left the audit trail expiring in seven days, so the write is now refused by name. A store that must hold one blob under two kinds needs per-kind metadata records, which is a design change rather than a silent collision. `pruneExpired` deletes artifacts past `retentionUntil` and only ever runs when called: a retention policy that fires on its own is an audit-trail hazard.
...
Reports are generated outside candidate code. The evaluator child runs with a timeout, Node's permission model, a heap ceiling and bounded output. CPU-time and network limits are not expressible in-process, so they come from a runtime the host names in configuration: `evaluation.sandboxCommand` and `evaluation.sandboxArgs` are a prefix, and the child is launched as `[...args, node, ...nodeArgs, entryPath]` with no shell in between. The plugin bundles no runtime and does not pretend to enforce what it cannot see — the runtime's own flags carry the limits, `dream_status` reports the prefix so an operator can read what confines a candidate, and arguments without a command are refused because they would confine nothing. This is what closes the isolation blocker's code side; evidence that a specific runtime confines a specific candidate is the host's, because it depends on what the host installs.
...
**Signature posture.** `SignatureVerifier` is a seam, and its absence is a refusal rather than a bypass: an unsigned artifact must not reach production because nobody wired the check. A verifier that *throws* — a provider that cannot reach its key service, say — is the same refusal and not an invitation to trust the artifact, which is the one failure mode the check exists to prevent. The signing scheme itself is an open blocker.
...
Each family carries quality, cost, latency, and miss rate as aggregates with sample counts and a 95% confidence interval (`src/observability/statistics.ts`). Small samples use a Student-t critical value rather than 1.96: at five samples the interval is 1.4x wider and at three samples 2.2x, which is the difference between "no regression" and "cannot tell yet". A single sample reports `lower` and `upper` as `null` rather than a zero-width interval, because one observation cannot support a precision claim. A non-finite sample is refused. A quantity a case did not record is absent from that quantity's aggregate and from its sample count, rather than counted as a zero — including `missed`, so a family whose cases never recorded it reports no miss rate instead of a clean one.
